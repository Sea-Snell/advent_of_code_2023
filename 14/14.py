...
def get_wild_hand_class(hand):
    if 'J' in hand:
        best_class = 7
        for wild_value in 'AKQT98765432':
            best_class = min(best_class, get_hand_class(hand.replace('J', wild_value)))
        return best_class
    else:
        return get_hand_class(hand)
...
if __name__ == "__main__":
    ...
    card_rank = {card: idx for idx, card in enumerate('AKQT98765432J')}
    hand_classes = [get_wild_hand_class(hand) for hand in hands]
